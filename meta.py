...
ENTRIES = [
    {
        ...
        "date": "2026-09-11",
        "uk": (
            "Велике оновлення — повноцінна оптимізація висоти (нова "
            "вкладка «Висота»), значні виправлення координатної "
            "оптимізації, і живий компас у HUD.\n\n"
            "**Оптимізація висоти (нове).** Після обходу заборонних зон "
            "по горизонталі — окремий алгоритм підганяє висоту вздовж "
            "усього маршруту під реальний рельєф (SRTM), ліміт кута "
            "набору/зниження з профілю апарату, і бюджет точок з типу "
            "маршруту. Працює послідовними фазами з поверненням: спершу "
            "усуваються критичні (від'ємний кліренс) порушення, потім "
            "повністю контролюється кут (зсувом наявних точок, а якщо "
            "точку затиснуто вимогами з обох боків — зміною її власної "
            "висоти), потім — недостатній кліренс (пріоритет підняттю "
            "наявних точок у межах кута, вставка нової — лише якщо цього "
            "не досить), і нарешті, поки лишається бюджет — щільніше "
            "прилягання до рельєфу дробленням найдовших ребер. Перехід "
            "через лінію розмежування — окремий, ізольований блок зі "
            "своїм лімітом кута (нове поле в «Тип маршруту»), щоб не "
            "плутатись зі звичайною логікою.\n\n"
            "Розрахунок виконується у фоновому потоці (інтерфейс "
            "лишається живим), є покроковий режим (пробіл/кнопка "
            "«Далі») з живими графіками «було/стало» й кута нахилу "
            "траєкторії, і кнопка «Скасувати» для дострокової зупинки. "
            "Звіт показує окремо найгірший кліренс і найгірший кут з "
            "точними координатами — легко знайти причину, якщо щось "
            "лишилось невиправленим.\n\n"
            "**Координатна оптимізація (виправлення).** Знайдено й "
            "виправлено серйозний регрес: фільтр, що прибирав фізично "
            "безглузді мікроребра між геть далекими перешкодами, у "
            "складних сценаріях (багато сіл поруч) міг узагалі не "
            "знаходити шлях обходу. Тепер — автоматичний відкат до "
            "повного графа, якщо з фільтром шлях не знайдено; "
            "коректність обходу більше ніколи не приноситься в жертву.\n\n"
            "**HUD (виправлення).** Курс (компас) уже надходив у потоці "
            "телеметрії VFR_HUD, але не малювався — додано стрічку "
            "курсу внизу штучного горизонту (N/E/S/W, поділки кожні "
            "10°), і числові поля путьової швидкості та вертикальної "
            "швидкості, які теж уже були в тому самому потоці.\n\n"
            "**Окремий інструмент.** Новий самостійний скрипт "
            "`clearance_distribution.py` (лише стандартна бібліотека) — "
            "будує розподіл кліренсу над рельєфом для однієї чи двох "
            "місій одразу, накладених на спільній координатній сітці, "
            "для наочного порівняння «було/стало» по всьому маршруту "
            "разом, не по окремих точках."
        ),
        "en": (
            "Major update — full altitude optimization (new \"Altitude\" "
            "tab), significant fixes to coordinate optimization, and a "
            "live compass in the HUD.\n\n"
            "**Altitude optimization (new).** After horizontal no-fly-"
            "zone avoidance, a separate algorithm fits the altitude "
            "along the entire route to the real terrain (SRTM), the "
            "climb/descent angle limit from the aircraft profile, and "
            "the point budget from the route type. It runs in "
            "sequential phases with feedback: first critical (negative "
            "clearance) violations are cleared, then the angle is fully "
            "controlled (by shifting existing points, or — if a point "
            "is pinched by requirements from both sides — by changing "
            "its own altitude instead), then insufficient clearance "
            "(priority given to raising existing points within the "
            "angle limit, a new point is only inserted if that's not "
            "enough), and finally, while budget remains, tighter "
            "terrain-hugging by splitting the longest remaining edges. "
            "Crossing the demarcation line is a separate, isolated "
            "block with its own angle limit (new field in \"Route "
            "type\"), so it never gets tangled up with the regular "
            "logic.\n\n"
            "The computation runs on a background thread (the interface "
            "stays responsive), there's a step-by-step mode (space bar "
            "or the \"Next\" button) with live before/after and flight-"
            "path-angle charts, and a \"Cancel\" button to stop early. "
            "The report shows the worst clearance and the worst angle "
            "separately, with exact coordinates — easy to find the "
            "cause if something is left unresolved.\n\n"
            "**Coordinate optimization (fix).** Found and fixed a "
            "serious regression: the filter that removed physically "
            "meaningless micro-edges between far-apart obstacles could, "
            "in complex scenarios (many nearby settlements), fail to "
            "find a detour path at all. Now there's an automatic "
            "fallback to the full graph if no path is found with the "
            "filter — correctness of the detour is never sacrificed "
            "again.\n\n"
            "**HUD (fix).** Heading (compass) data was already arriving "
            "in the VFR_HUD telemetry stream but was never drawn — "
            "added a heading tape below the artificial horizon (N/E/S/W, "
            "10° ticks), plus numeric ground speed and climb rate "
            "readouts, which were also already in the same stream.\n\n"
            "**Separate tool.** New standalone script "
            "`clearance_distribution.py` (standard library only) — "
            "builds a terrain clearance distribution for one or two "
            "missions at once, overlaid on a shared coordinate grid, "
            "for an at-a-glance before/after comparison across the "
            "whole route rather than individual points."
        ),
    },
    {
        "version": "1.0.6",
