...
# ============================================================
# Єдині кольори/шрифт/форма стрілки для ВСІХ оверлеїв НА КАРТІ
# (компас+стрілка вітру й азимуту польоту -- Зліт/Посадка/Маршрут,
# позначки обходу НП -- Обліт НП/Оптимізація тощо). НЕ залежить від
# dark/dark -- це кольори для контрасту проти супутникового знімка під
# ними, а не проти фону самого застосунку (той не змінюється залежно
# від теми програми, тому й кольори лишаються сталими в обох темах).
#
# Виявлено на практиці: до появи цього єдиного джерела стрілка вітру
# на "Маршруті" й на "Зльоті/Посадці" мали РІЗНУ довжину й розмір
# шрифту, хоча малювали один і той самий фізичний показник -- просто
# кожне місце рахувало власні числа окремо. Будь-яка НОВА позначка на
# карті має брати кольори/шрифт/форму СТРІЛКИ звідси, а не вигадувати
# власні.
MAP_OVERLAY_COLORS = {
    "wind": "#00BFFF",
    "flight_az": "#39FF14",
...
MAP_OVERLAY_LABEL_FONT_SIZE = 11   # підпис біля стрілки (швидкість/напрямок/час)
MAP_OVERLAY_SMALL_FONT_SIZE = 8    # дрібніші підписи (N/E/S/W компаса тощо)
MAP_ARROW_WIDTH = 3
MAP_ARROW_SHAPE = (12, 14, 5)      # tk create_line(arrowshape=...)


def map_overlay_font(size: int = MAP_OVERLAY_LABEL_FONT_SIZE) -> tuple:
    """Шрифт для підписів на карті -- та сама родина (_FONT_FAMILY), що
    й у решті інтерфейсу, просто з явним розміром під конкретний підпис."""
    return (_FONT_FAMILY, size, "bold")


MAP_LABEL_BACKDROP_ALPHA = 140  # 0-255, наскільки темна підкладка під текстом


def draw_label_backdrop(canvas, bbox: tuple, image_refs: list, tag: str = "overlay_layer") -> None:
    """Напівпрозора чорна підкладка під підписом на карті (bbox --
    canvas.bbox(text_id), (x0,y0,x1,y1)) -- ЄДИНА реалізація для ВСІХ
    підписів на будь-якій карті (Зліт/Посадка/Маршрут/Обліт НП/
    Оптимізація), щоб текст не губився на строкатому супутниковому
    знімку під ним.

    Справжня альфа-прозорість через PIL (create_image), НЕ
    canvas.create_rectangle(..., stipple="gray50") -- останній дає
    грубий пунктирний патерн (не плавну прозорість), і коли поруч
    опиняється кілька таких підкладок (напр. кілька позначок вітру на
    "Маршруті") -- вони візуально зливаються в суцільну темну пляму,
    а не виглядають як кілька акуратних напівпрозорих ярликів.

    tag -- ВИЯВЛЕНО НА ТЕСТАХ: якщо жорстко прошити "overlay_layer"
    всередині цієї функції, підкладка на "Маршруті" (де своя логіка
    очищення/перемальовки прив'язана до тегу "route_wind") малюється
    коректно, але з ЧУЖИМ тегом -- і код, що шукає елементи за
    "route_wind" (напр. для очищення перед перемальовкою), її просто
    не бачить і не видаляє при оновленні. Виклик МАЄ передавати СВІЙ
    тег, що використовується на цьому конкретному canvas.

    image_refs -- список (звичайний Python list), куди ОБОВ'ЯЗКОВО
    треба покласти повернутий PhotoImage -- Tkinter не тримає власного
    посилання на зображення canvas, без зовнішнього списку об'єкт
    негайно збирається сміттярем і підкладка зникає з екрана."""
    x0, y0, x1, y1 = bbox
    x0, y0, x1, y1 = x0 - 3, y0 - 2, x1 + 3, y1 + 2
    ...
    try:
        ...
        img = Image.new("RGBA", (w, h), (0, 0, 0, MAP_LABEL_BACKDROP_ALPHA))
        photo = ImageTk.PhotoImage(img)
        image_refs.append(photo)  # інакше збереться сміттярем одразу
        canvas.create_image(x0, y0, image=photo, anchor="nw", tags=tag)
    except ImportError:
        # PIL відсутній (опційна залежність, див. README) -- запасний
        # варіант через stipple, гірше виглядає, але не падає зовсім
        canvas.create_rectangle(
            x0, y0, x1, y1, fill="#000000", outline="", stipple="gray50",
            tags=tag,
        )
