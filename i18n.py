...
_TR: dict[str, dict[str, str]] = {
    ...
    "help_text_body": {
        "uk": (
            "MISSION ANALYZER \u2014 аналіз і оптимізація місій ArduPilot / Mission Planner (.waypoints)\n\n"
            "СТОРІНКИ\n"
            ...
            "Аналіз \u2014 п'ять вкладок: Зліт і Посадка (погода, вітер, глісада), Маршрут (карта "
            "згори, графіки висоти й кута траєкторії, вітер у кількох точках уздовж маршруту за "
            "часом польоту), Обліт НП (перевірка відстані до населених пунктів по кожному "
            "відрізку маршруту, не тільки по точках) і Оптимізація (автоматична побудова обходу "
            "населених пунктів \u2014 див. розділ нижче).\n\n"
            ...
            "окупованих територій, профілі літака (див. розділ нижче).\n\n"
            "Довідка \u2014 цей текст.\n\n"
            "РЕДАГУВАННЯ МІСІЇ\n"
            ...
            "ОБЛІТ НАСЕЛЕНИХ ПУНКТІВ\n"
            "На вкладці «Обліт НП» застосунок запитує реальні координати сіл і міст поблизу "
            "маршруту (Overpass API, OpenStreetMap) і рахує мінімальну відстань від кожного "
            "відрізка маршруту (не тільки від самих вейпоінтів) до найближчого населеного пункту. "
            "Порушення порогу (типово 1 км, можна змінити) позначаються на карті кольором і в "
            "таблиці \u2014 з номером ребра, назвою, відстанню й позначкою лівого/правого боку "
            "відносно напрямку польоту.\n\n"
            "ОПТИМІЗАЦІЯ МАРШРУТУ\n"
            "Якщо маршрут проходить занадто близько до населеного пункту, вкладка «Оптимізація» "
            "може побудувати обхід автоматично \u2014 геометрично найкоротший шлях навколо всіх "
            "перешкод одразу (метод графа дотичних), а не послідовними латками. Перед розрахунком "
            "перевіряються: паливний бюджет (ємність бака, витрата, резерв за стандартом ICAO "
            "Annex 6), ліміт кількості точок місії (255) і фізична можливість літака виконати "
            "такий поворот (радіус повороту від швидкості й максимального крену \u2014 якщо замалий, "
            "застосунок чесно про це повідомить, а не мовчки видасть непридатний маршрут). "
            "Результат \u2014 карта «було/стало» (обидва маршрути одна поверх одної), таблиця "
            "відстаней до кожного населеного пункту до й після, і кнопка збереження оптимізованого "
            "маршруту в окремий .waypoints-файл (оригінал не змінюється).\n\n"
            "ПРОФІЛЬ ЛІТАКА\n"
            "На сторінці «Конфігурація» можна один раз заповнити профіль літака (крейсерська "
            "швидкість, обмеження крену й тангажу, витрата палива, швидкість зниження/набору) \u2014 "
            "далі «Оптимізація» бере ці значення з випадаючого списку профілю замість того, щоб "
            "вводити їх вручну щоразу. Профілі розрізняються за типом апарата (Літак/Коптер/"
            "Ровер/Підводний апарат, як у Mission Planner) \u2014 наразі повний набір польотних "
            "полів реалізований лише для типу «Літак».\n\n"
            "ARDUPILOT\n"
            ...
            "версія прошивки, тип і серійний номер плати, датчики, батарея, HUD, скан Lua-скриптів "
            "на SD-карті. Команди \u2014 перевіряє, які з ~186 команд місії борт реально приймає "
            "(пробує наживо, не покладається на документацію). Параметри \u2014 ключові параметри "
            "залежно від типу апарата (Plane/Copter/Rover/Sub), включно зі швидкостями, кутами й "
            "параметрами TECS (глісада, набір висоти). Файли SD \u2014 перегляд, скачування, "
            ...
            "- близькість маршруту до населених пунктів (Обліт НП)\n\n"
            "НАЛАШТУВАННЯ\n"
            "Обрані папки, пороги, зум, мова тощо запам'ятовуються автоматично між запусками "
            "(файл settings.json поруч із програмою). Профілі літака зберігаються окремо "
            "(aircraft_profiles.json). Щоб скинути налаштування на типові значення \u2014 просто "
            "видали settings.json.\n\n"
        ...
        "en": (
            "MISSION ANALYZER \u2014 ArduPilot / Mission Planner (.waypoints) mission analysis and optimization\n\n"
            "PAGES\n"
            ...
            "Analysis \u2014 five tabs: Takeoff and Landing (weather, wind, glide slope), Route (top-"
            "down map, altitude and flight-path-angle charts, wind at several points along the "
            "route by flight time), Settlements (checks distance to populated areas for every "
            "route leg, not just the waypoints), and Optimization (automatic detour building "
            "around populated areas \u2014 see below).\n\n"
            ...
            "territories layer, aircraft profiles (see below).\n\n"
            "Help \u2014 this text.\n\n"
            "EDITING A MISSION\n"
            ...
            "The Settlements tab queries real coordinates of villages and towns near the route "
            "(Overpass API, OpenStreetMap) and computes the minimum distance from every route "
            "leg (not just the waypoints) to the nearest settlement. Violations of the threshold "
            "(1 km by default, adjustable) are marked on the map by color and in a table \u2014 with "
            "the leg number, name, distance, and which side of the flight direction it's on.\n\n"
            "ROUTE OPTIMIZATION\n"
            "If the route passes too close to a settlement, the Optimization tab can build a "
            "detour automatically \u2014 the geometrically shortest path around every obstacle at "
            "once (tangent visibility graph), not a series of local patches. Before the result "
            "ships, it checks: fuel budget (tank capacity, consumption, ICAO Annex 6 reserve), "
            "the mission waypoint-count limit (255), and whether the airframe can physically fly "
            "the required turn (radius from airspeed and max bank angle \u2014 if too tight, the "
            "tool says so plainly instead of silently handing over an unflyable route). The result "
            "is a before/after map (both routes overlaid) plus a per-settlement distance table, "
            "and a button to save the optimized route as a new .waypoints file (the original is "
            "never touched).\n\n"
            "AIRCRAFT PROFILE\n"
            "On the Configuration page you can fill in an aircraft profile once (cruise airspeed, "
            "bank/pitch limits, fuel consumption, sink/climb rate) \u2014 Optimization then pulls "
            "these values from a dropdown instead of asking you to type them in every time. "
            "Profiles are split by vehicle type (Plane/Copter/Rover/Sub, matching Mission "
            "Planner) \u2014 right now the full set of flight fields is implemented only for Plane.\n\n"
            "ARDUPILOT\n"
            ...
            "firmware version, board type and serial number, sensors, battery, HUD, a scan of Lua "
            "scripts on the SD card. Commands \u2014 checks which of the ~186 mission commands the "
            "board actually accepts (tests live, doesn't trust documentation). Parameters \u2014 key "
            "parameters depending on vehicle type (Plane/Copter/Rover/Sub), including airspeeds, "
            "angle limits, and TECS parameters (glide slope, climb rate). SD Files \u2014 browse, "
            "download, upload, delete files, and plot flight dataflash logs (.BIN).\n\n"
            "WHAT IS CHECKED\n"
            ...
            "- route proximity to populated areas (Settlements)\n\n"
            "SETTINGS\n"
            "Chosen folders, thresholds, zoom, language etc. are remembered automatically between "
            "runs (settings.json next to the program). Aircraft profiles are stored separately "
            "(aircraft_profiles.json). Delete settings.json to reset everything to defaults.\n\n"
