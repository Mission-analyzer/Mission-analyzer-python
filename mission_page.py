...
from map_view import (
    ...
    render_tiles_fit, render_viewport, MapTooLargeError,
)
...
class MissionPageMixin:
    ...
    def _on_tiles_ready(
        ...
        # Кожен тайл окремо, 1:1, без склейки в мозаїку й без
        # PIL.resize() -- як у Mission Planner (GMap.NET). Ніякого
        # додаткового масштабування -- зум уже підібраний правильно
        # (_find_native_fit_zoom), більше нічого домальовувати не треба.
        found, total, missing, undecodable, origin_gx, origin_gy = render_viewport(
